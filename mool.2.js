...
var C=this.replace(/<script[^>]*>([sS]*?)</script>/gi,function(){A+=arguments[1]+”n”;return””;});if(B===true){$exec(A);}else{if($type(B)==”function”){B(A,C);
...
}for(var E in Element.ShortStyles[F]){A.push(this.getStyle(E));}return A.join(” “);}A=this.getComputedStyle(G);}if(A){A=String(A);var C=A.match(/rgba?([ds,]+)/);
...
case false:return”null”;}return null;},$specialChars:{“b”:”\b”,”t”:”\t”,”n”:”\n”,”f”:”\f”,”r”:”\r”,'”‘:’\”‘,”\”:”\\”},$replaceChars:function(A){return JSON.$specialChars[A]||”\u00″+Math.floor(A.charCodeAt()/16).toString(16)+(A.charCodeAt()%16).toString(16);
},decode:function(string,secure){if($type(string)!=”string”||!string.length){return null;}if(secure&&!(/^[,:{}[]0-9.-+Eaeflnr-u nrt]*$/).test(string.replace(/\./g,”@”).replace(/”[^”\nr]*”/g,””))){return null;
...
}};Element.implement({send:function(A){var B=this.get(“send”);B.send({data:this,url:A||B.options.url});return this;}});Request.HTML=new Class({Extends:Request,options:{update:false,evalScripts:true,filter:false},processHTML:function(B){var A=B.match(/<body[^>]*>([sS]*?)</body>/i);
...
}B.removeProperty(“title”);}else{B.$attributes.myText=false;}if(B.$attributes.myTitle&&B.$attributes.myTitle.length>this.options.maxTitleChars){B.$attributes.myTitle=B.$attributes.myTitle.substr(0,this.options.maxTitleChars-1)+”…”;
